    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let error = errorMessage {
                ...
                let workday: TimeInterval = 7 * 3600
                if let os = service.openShift {
                    TimelineView(.periodic(from: .now, by: 1.0)) { tl in
                        let elapsed = min(tl.date.timeIntervalSince(os.clockIn), workday)
                        TodayProgressRow(elapsed: elapsed, total: workday, isLive: true)
                    }
                } else if let completed = service.todayCompletedDuration {
            ...
            // Fichar ahora / Timer en curso
            if let os = service.openShift {
                TimelineView(.periodic(from: .now, by: 1.0)) { tl in
                    let elapsed = tl.date.timeIntervalSince(os.clockIn)
                    let remaining = max(0, 7 * 3600 - elapsed)
                    HStack(spacing: 10) {
